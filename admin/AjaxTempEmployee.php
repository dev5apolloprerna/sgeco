...
if ($_POST['action'] == 'ListUser') {
    ...
    if (isset($_REQUEST['Search_From_Date'])) {
        if ($_POST['Search_From_Date'] != '') {
            $where.=" and  STR_TO_DATE(strEntryDate,'%d-%m-%Y %H:%i:%s') >= STR_TO_DATE('" . $_POST['Search_From_Date'] . " 00:00:00','%d-%m-%Y')";
        }
    }
    if (isset($_REQUEST['Search_To_Date'])) {
        if ($_POST['Search_To_Date'] != '') {
            $where.=" and  STR_TO_DATE(strEntryDate,'%d-%m-%Y %H:%i:%s') <= STR_TO_DATE('" . $_POST['Search_To_Date'] . " 23:59:59','%d-%m-%Y') ";
        }
    }

    $filterstr = "SELECT * FROM `tempEmpolyeeMaster`  " . $where . " and isDelete='0'  and  istatus='1' order by iTempEmpId desc";
                    ...
                    <tbody>
                        ...
                        while ($rowfilter = mysqli_fetch_array($resultfilter)) {
                            ...
                            <tr>
                                ...
                                <?php $filterFamilyRelation = mysqli_query($dbconn,"SELECT * FROM `tempFamilyDetails` where iTempEmpId='".$rowfilter['iTempEmpId']."' order by iTempFamilyDetailsId asc limit 1");
                                    $strFamilyDetails = "";
                                    $strRelation = "";
                                ...
                                <td>
                                    <div class="form-group form-md-line-input "><?php echo ucwords(strtolower($CreatedBy['strUserName'])); ?> 
                                    </div>
                                </td>
                                ...
                                <td style="width: 20%">
                                    <div class="form-group form-md-line-input">
                                        <!--<a  class="btn blue" href="<?php echo $web_url; ?>admin/EditEmployee.php?token=<?php echo $rowfilter['iTempEmpId']; ?>" title="Edit"><i class="fa fa-edit iconshowFirst"></i></i></a>-->
                                        <a  class="btn blue" onClick="javascript: return deletedata('Delete', '<?php echo $rowfilter['iTempEmpId']; ?>');"   title="Delete"><i class="fa fa-trash-o iconshowFirst"></i></a>
                                        <!--<a  class="btn blue" href="<?php echo $web_url; ?>admin/EmployeeLedger.php?token=<?php echo $rowfilter['iTempEmpId']; ?>" title="Ledger"><i class="fa fa-user"></i></i></a>-->
                                        ...
                                        <a  class="btn blue" href="<?php echo $web_url; ?>admin/ViewTempDocument.php?token=<?php echo $rowfilter['iTempEmpId']; ?>" title="View Document"><i class="fa fa-eye"></i></i></a>
                                        <a  class="btn blue" href="<?php echo $web_url; ?>admin/DownloadTempDocument.php?token=<?php echo $rowfilter['iTempEmpId']; ?>" title="Download Document"><i class="fa fa-download"></i></i></a>
                                        <a  class="btn blue" href="<?php echo $web_url; ?>admin/moveTempEmployee.php?token=<?php echo $rowfilter['iTempEmpId']; ?>" title="Edit"><i class="fa fa-edit iconshowFirst"></i></i></a>
                                    </div>
                                </td>
...
if ($_REQUEST['action'] == 'Delete') {
    ...
    $filterSql = mysqli_query($dbconn, 'select * from temempdocument where iTempEmpId in  ("' . $_REQUEST['ID'] . '")');
    if(mysqli_num_rows($filterSql) > 0)
    {
        while($row = mysqli_fetch_assoc($filterSql)){
            $path = "./TempDocument/".$_REQUEST['ID'] ."/".$row['strDocumentImage'];
            if(file_exists($path)){
                unlink($path);    
            }
        }
    }
    mysqli_query($dbconn, 'delete from temempdocument where iTempEmpId in  ("' . $_REQUEST['ID'] . '")');
    mysqli_query($dbconn, 'delete from tempEmpolyeeMaster where iTempEmpId="' . $_REQUEST['ID'] . '"');
}
?>
...
<script>
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response == 1)
                    {
                        ...
                        alert('Move Sucessfully.');
                        window.location.href = '';
                    } else
                    {
                        $('#loading').css("display", "none");
                        $("#Btnmybtn").attr('disabled', 'disabled');
                        alert('Not Move  Please Try Again.');
                        window.location.href = '';
                    }
                }
    ...
    function ViewExportList(){
        window.location.href = 'ViewExportList.php?token=2';
    }
