...
if ($_REQUEST['Company'] != NULL  && $_REQUEST['salaryId'] != NULL) {
    ...
    $filterstr = "SELECT salarydetails.salarydetailsId,salarydetails.salaryId,emp_name,accountno,companypaymentMaster.salarymonth,companypaymentMaster.iBank,companypaymentMaster.strTransactionNo,companypaymentMaster.iPaymentMode,companypaymentMaster.iAmount,
        ...
        CASE
            ...
            ELSE 'Bank'
        END as strPaymentMode,netamountpaid,emp_other_info,ifsccode,companypaymentMaster.strPaymentDate FROM employee INNER JOIN salarydetails ON employee.employeeId=salarydetails.emp_id inner JOIN companypaymentMaster on companypaymentMaster.iCompanyPaymentId=salarydetails.iPaymentId where salarydetails.companyId='" . $_REQUEST['Company'] . "' and salarydetails.salaryId in (select salarymasterId from salarymaster where  month='" . $_REQUEST['salaryId'] . "' and isDelete='0' and  istatus='1') and salarydetails.workingdays > 0  " . $where . " and iPaymentStatus=1 and  employee.isDelete = '0' and employee.istatus= '1'  ORDER BY `emp_name` ASC";
    //exit;
...
$Total = array(0, 0);
$mailFormat_main = file_get_contents("companyPaidPayment.html");
$i = 1;
$mailFormat_rows = "";
...
while ($rowapplication = mysqli_fetch_array($result)) {
    ...
    $mailFormat = file_get_contents("companyPaidPaymenttr.html");
    ...
    $mailFormat = str_replace("#netamountpaid#", ucfirst(urldecode(number_format( ceil($rowapplication['iAmount']) ,2))), $mailFormat);
    ...
    $mailFormat = str_replace("#Month#", ucfirst(urldecode($rowapplication['salarymonth'])), $mailFormat);
    $mailFormat = str_replace("#Mode#", ucfirst(urldecode($rowapplication['strPaymentMode'])), $mailFormat);
    $mailFormat = str_replace("#Bank#", ucfirst(urldecode($rowapplication['PaidBankName'])), $mailFormat);
    $mailFormat = str_replace("#TransactionNo#", ucfirst(urldecode($rowapplication['strTransactionNo'])), $mailFormat);
    $mailFormat = str_replace("#PaidDate#", ucfirst(urldecode($rowapplication['strPaymentDate'])), $mailFormat);
    $mailFormat_rows = $mailFormat_rows . $mailFormat;
    $i++;
    echo $rowapplication['salarymonth'];
}
