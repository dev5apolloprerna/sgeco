...
$filterstr = "SELECT salarydetails.salarydetailsId,salarydetails.salaryId,emp_name,accountno,companypaymentMaster.salarymonth,companypaymentMaster.iBank,companypaymentMaster.strTransactionNo,companypaymentMaster.iPaymentMode,companypaymentMaster.iAmount,
        ...
        CASE
            ...
            ELSE 'Bank'
        END as iPaymentMode,netamountpaid,emp_other_info,ifsccode,companypaymentMaster.strPaymentDate FROM employee INNER JOIN salarydetails ON employee.employeeId=salarydetails.emp_id inner JOIN companypaymentMaster on companypaymentMaster.iCompanyPaymentId=salarydetails.iPaymentId where salarydetails.companyId='" . $_REQUEST['Company'] . "' and salarydetails.salaryId in (select salarymasterId from salarymaster where  month='" . $_REQUEST['salaryId'] . "' and isDelete='0' and  istatus='1') and salarydetails.workingdays > 0  " . $where . " and iPaymentStatus=1 and  employee.isDelete = '0' and employee.istatus= '1'  ORDER BY `emp_name` ASC";
        ...
        //if ($Balance > 0) {
             if ($_REQUEST['bank'] == 3) {
                $lineData = array(
                    ...
                    date('d-m-Y',strtotime($row['strPaymentDate']))
...
//move back to beginning of file
    ...
    $filename = "CompanyPaidPaymentReport" . date('Y-m-d H:i:s') . ".csv";
