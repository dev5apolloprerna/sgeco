...
switch ($action) {
    ...
    case "AddBank":
        ...
        if (mysqli_num_rows($chankbank) == 0) {
            $data = array(
                "bankname" => ucwords(strtolower($_POST['Bank'])),
    ...
    case "AddCompany":
        ...
        if (mysqli_num_rows($chankcomp) == 0) {
            $data = array(
                "companyname" => ucwords(strtolower($_POST['Company'])),
                "skil" => $_POST['Skil'],
    ...
    case "AddCompanySalary":
        ...
        foreach ($Company as $key => $value) {
            mysqli_query($dbconn, "INSERT INTO `multiycompanysalarymaster`(companysalarymasterId,companymasterId,strEntryDate,strIP) VALUES ('" . $dealer_res . "','" . $value . "', '" . date('d-m-Y H:i:s') . "', '" . $_SERVER['REMOTE_ADDR'] . "' ) ");
        }
        echo $statusMsg = $dealer_res ? '1' : '0';
    ...
    case "EditCompanySalary":
        ...
        while ($rowC = mysqli_fetch_array($resultCategory)) {
            ...
            //   mysql_query("INSERT INTO `clientemployeecategory`(iclientEmployeeId,iCategoryId,strEntryDate,strIP) VALUES ('" . $dealer_res . "','" . $rowC['iCategoryId'] . "', '" . date('d-m-Y H:i:s'). "', '" . $_SERVER['REMOTE_ADDR'] . "' ) ")or die(mysql_error());
                mysqli_query($dbconn, "INSERT INTO `multiycompanysalarymaster`(companysalarymasterId,companymasterId,strEntryDate,strIP) VALUES ('" . $_REQUEST['companysalarymasterId'] . "','" . $rowC['companymasterId'] . "', '" . date('d-m-Y H:i:s') . "', '" . $_SERVER['REMOTE_ADDR'] . "' ) ");
        }
        echo $statusMsg = $dealer_res ? '2' : '0';
        break;
...
//        $path = 'Document/'; // upload directory
        ...
        if($_POST['mno'] != ""){
            $filterTempEmpMno = mysqli_query($dbconn,"SELECT mno from employee where mno='".$_POST['mno']."' and isDelete=0 and istatus=1");
            if(mysqli_num_rows($filterTempEmpMno) > 0){
                echo 4;
            }
        }
        if($_POST['ecsno'] != ""){
            $filterTempEmpEcsno = mysqli_query($dbconn,"SELECT ecsno from employee where ecsno='".$_POST['ecsno']."' and isDelete=0 and istatus=1");
        ...
        // }
        if($_POST['pfcode'] != ""){
            $filterTempEmpPFCode = mysqli_query($dbconn,"SELECT pfcode from employee where pfcode='".$_POST['pfcode']."' and isDelete=0 and istatus=1");
        ...
        if($_POST['uan'] != ""){
            $filterTempEmpUan = mysqli_query($dbconn,"SELECT uan from employee where uan='".$_POST['uan']."' and isDelete=0 and istatus=1");
        ...
        if($_POST['pancard'] != ""){
            $filterTempEmpPancard = mysqli_query($dbconn,"SELECT pancard from employee where pancard='".$_POST['pancard']."' and isDelete=0 and istatus=1");
        ...
        if($_POST['aadharcard'] != ""){
            $filterTempEmpAadharCard = mysqli_query($dbconn,"SELECT adharcard from employee where adharcard='".$_POST['aadharcard']."' and isDelete=0 and istatus=1");
        ...
        $emp = mysqli_query($dbconn, "SELECT * FROM employee where  emp_name = '" . $_POST['emp_name'] . "' and isDelete='0' and istatus='1' ");
...
//                    $ecs = round($total) * 0.0175;
                ...
                $dataarray = array(
                    ...
                    "salarypaiddate" => $_POST['salarypaiddate'],
                    "emp_id" => $_POST['emp_id_' . $inc],
                    "name" => ucwords(strtolower($_POST['Ename_' . $inc])),
    ...
    case "multicompany":
        ...
        for ($iCounter = 0; $iCounter < $count; $iCounter++) {
            $dataarray = array
                ...
                "name" => ucwords(strtolower($_POST['name'][$iCounter])),
                "salaryamt" => $_POST['salaryamt'][$iCounter],
...
//                "pfcode" => $_POST['pfcode'][$iCounter],
    ...
    case "editProfile":
        $data = array(
            "strCompanyName" => ucwords(strtolower($_POST['strCompanyName'])),
            "strAddress" => ucwords(strtolower($_POST['strAddress'])),
            ...
            "strBankName" => ucwords(strtolower($_POST['strBankName'])),
            "strBranchNameandAddress" => ucwords(strtolower($_POST['strBranchNameandAddress'])),
            "IFSCCode" => $_POST['IFSCCode'],
