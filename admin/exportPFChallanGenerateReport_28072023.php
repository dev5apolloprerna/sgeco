...
$sql1 = "SELECT employee.employeeId,max(skillrate) as skillrate,(max(skillrate)- min(skillrate)) as Diff,employee.employeecode,
        ...
        FROM `salarydetails` inner join employee on salarydetails.emp_id=employee.employeeId where  salaryId in (select salarymasterId from salarymaster where  month='".$salaryMonth."' and isDelete='0' and  istatus='1') and  salarydetails.isDelete='0'  and salarydetails.istatus='1' and salarydetails.workingdays > 0 and employee.employeecode !=0 GROUP by employee.employeeId order by employee.employeecode asc";
...
if (mysqli_num_rows($result1) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($tempEmpolyeeMasterResult)) {
        ...
        $dateofjoining = isset($row['dateofjoining']) && $row['dateofjoining'] != "" ? date('d-m-Y',strtotime(trim($row['dateofjoining']))) :"";
        // $DifferenceInESIC=0;
        // if($row['Diff'] == 0){
