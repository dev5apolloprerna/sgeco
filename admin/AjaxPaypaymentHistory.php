...
if ($_POST['action'] == 'ListUser') {
    ...
    if ($_REQUEST['companysalarymasterId'] != NULL ){
        $where .= " and paymentMaster.iCompanySalaryMasterId = " . $_POST['companysalarymasterId'] . " ";
    }
    if ($_REQUEST['salarymonthId'] != NULL) {
        $where .= " and paymentMaster.salarymonth = '" . $_POST['salarymonthId'] . "'";
    }
    if ($_REQUEST['bank'] != NULL) {
        $where .= " and paymentMaster.iBank = " . $_POST['bank'] . "";
    }
    ...
    $countstr = "SELECT count(*) as TotalRow FROM `paymentMaster`,companysalarymaster where companysalarymaster.companysalarymasterId = paymentMaster.iCompanySalaryMasterId" . $where . " and paymentMaster.isDelete='0' and paymentMaster.iStatus='1' ";
            ...
            <tbody>
                ...
                while ($rowfilter = mysqli_fetch_array($resultfilter)) {
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="form-group form-md-line-input "><?php echo $rowfilter['iPaymentMode'] == 1 ? 'Cash' : 'Bank'; ?> 
                                </div>
                            </td> 
                            <td>
                            ...
                            <td>
                                <div class="form-group form-md-line-input ">
                                    <a  class="btn blue" href="#" onclick="updatepaymentDate(<?php echo $rowfilter['iPaymentId']; ?>,'<?php echo $rowfilter['strPaymentDate']; ?>');" data-toggle="modal" data-target="#exampleModal" title="Update Payment Date"><i class="fa fa-edit iconshowFirst"></i></i></a>
                                    <a  class="btn blue" href="<?php echo $web_url; ?>admin/ViewMultiCompanyPaypaymentHistory.php?token=<?php echo $rowfilter['iPaymentId']; ?>" title="View History"><i class="fa fa-eye"></i></i></a>
                                    <a  class="btn blue" onClick="javascript: return deletedata('Delete', '<?php echo $rowfilter['iPaymentId']; ?>');"   title="Delete"><i class="fa fa-trash-o iconshowFirst"></i></a>
                                </div>
                            </td> 
                        </tr>
                        <?php
...
if ($_POST['action'] == 'Delete') {
    ...
    mysqli_query($dbconn, 'update multicompany set iPaymentId=0,iPaymentStatus=0,iPaymentBy=0,strPaymentDate="" where iPaymentId="'.$_REQUEST['ID'].'"');
    $data = array(
        "isDelete" => '1',
        "strEntryDate" => date('d-m-Y H:i:s')
    );
    $where = ' where iPaymentId=' . $_REQUEST['ID'];
    $dealer_res = $connect->updaterecord($dbconn,'paymentMaster', $data, $where);    
    
    
    $salaery = mysqli_fetch_assoc(mysqli_query($dbconn,"select *,(select GROUP_CONCAT(companymaster.companyname SEPARATOR ',') as Company from companymaster,multiycompanysalarymaster where companymaster.companymasterId= multiycompanysalarymaster.companymasterId
  	and multiycompanysalarymaster.companysalarymasterId in (select pm.iCompanySalaryMasterId from paymentMaster as pm where pm.iCompanySalaryMasterId=paymentMaster.iCompanySalaryMasterId) and multiycompanysalarymaster.isDelete=0
  	group by multiycompanysalarymaster.companysalarymasterId) as companyname from paymentMaster where iPaymentId='".$_REQUEST['ID']."'"));
    $msg = "Dear Sir, <br /><br />
    ...
    $connect->sendmail($msg, "sgeco@rediffmail.com", $sub = 'Remove Multicompany Salary History', $mailHost, $mailFrom, $mailFromName, $mailSMTPSecure, $mailAddReplyTo, $mailUsername, $mailPassword);
}
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <form  role="form"  method="POST"  action="" name="frmchangepwd"  id="frmchangepwd" enctype="multipart/form-data">
                <input type="hidden" value="AddChangePaymentDate" name="action" id="action">
                <input type="hidden" value="" name="iPaymentId" id="iPaymentId">
...
function updatepaymentDate(iPaymentId,strPaymentDate){
    $("#iPaymentId").val(iPaymentId);
