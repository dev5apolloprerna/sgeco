...
function formXXIIIFormatRoundedDownAmount($number)
{
    return formXXIIIFormatAmount(floor((float) $number));
}
...
function renderFormXXIIIHtml(array $entries, $salaryMonth, $companyName, $repeatPageHeaders = true)
{
    ...
    foreach ($entries as $index => $entry) {
        ...
        $values = array(
            ...
            formXXIIIFormatRoundedDownAmount($entry['totalovertime']),
        ...
        // The payable overtime earning is rounded down to the lower whole
        // amount. Add the displayed values so the register total agrees in
        // the list and in its Excel and PDF exports.
        $totals['earnings'] += floor((float) $entry['totalovertime']);
    }
    $totalValues = array(
        '',
        '<strong>TOTAL</strong>',
