...
if ($_REQUEST['bank'] != NULL) {
    if ($_REQUEST['bank'] == 3) {
        ...
        $where .= " and employee.bankid = " . $_REQUEST['bank'] . " and multicompany.pay_cash='0'";
    } else {
...
$Total = array(0, 0);
$mailFormat_main = file_get_contents("paidpayment.html");
$i = 1;
$mailFormat_rows = "";
...
//    $mailFormat_main = str_replace("#Company#", ucfirst(urldecode($comp['companyname'])), $mailFormat_main);
    ...
    if ($Balance > 0) {
        $Total[1] = $Balance + $Total[1];
        $mailFormat = file_get_contents("paidpayment_tr.html");
        ...
        $mailFormat = str_replace("#Mode#", ucfirst(urldecode($rowapplication['iPaymentMode'])), $mailFormat);
        $mailFormat = str_replace("#PaidBankName#", ucfirst(urldecode($rowapplication['PaidBankName'])), $mailFormat);
        $mailFormat = str_replace("#strTransactionNo#", ucfirst(urldecode($rowapplication['strTransactionNo'])), $mailFormat);
        $mailFormat = str_replace("#PaymentDate#", ucfirst(urldecode(date('d-m-Y',strtotime($rowapplication['strPaymentDate'])))), $mailFormat);
...
$pdf->Output('PaidpaymentReport.pdf', 'I');
?>